...
test('data/kits.json is generated, and matches the specs on disk', () => {
  // Hand-maintaining this list would let the site advertise a kit that no longer
  // exists, or hide one that does.
  build();
  execFileSync(NODE, [path.join(ROOT, 'kits', 'build.js')], { stdio: 'ignore' });
  const listed = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'kits.json'), 'utf8'));
  assert.match(listed._comment, /Generated by kits\/build\.js/);
  assert.equal(Object.keys(listed.kits).length, specs.length);
  for (const s of specs) {
    assert.ok(listed.kits[s.escape], `${s.id} is not advertised`);
    assert.equal(listed.kits[s.escape].id, s.id);
  }
});
